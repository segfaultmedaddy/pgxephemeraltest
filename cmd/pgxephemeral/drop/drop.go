...
func New() *cli.Command {
	...
	return &cli.Command{
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			return cmdutil.Write(drop(ctx, args{
...
func drop(ctx context.Context, args args) (any, error) {
	...
	if len(toDrop) == 0 {
		return nil, errors.New("no matching databases to drop")
	}

	names := slices.Collect(maps.Keys(toDrop))
	if err := m.DropDBs(ctx, names); err != nil {
		if args.All {
			return nil, fmt.Errorf("drop all selected databases: %w", err)
		}

		return nil, fmt.Errorf("drop selected databases: %w", err)
	}

	return slices.Collect(maps.Values(toDrop)), nil
}
